...
### Added
- **Configurable message size limit**: `ReplicationConfig::max_message_size` / `with_max_message_size()` bound the largest replication message the worker accepts. The payload buffer is sized from the length the server announces, so this caps the memory a single message can take; a larger message ends the stream with a protocol error. `MessageReader::with_max_message_size()` exposes the same limit for direct use. Defaults to the existing 1 GiB `MAX_MESSAGE_SIZE`, so behavior is unchanged unless set.
